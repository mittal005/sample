...
if ($Status=='Publish') {
  $query="INSERT INTO posts(user_id,username,Title,Content,Status,publishTime,Visibility,publihedOn,Format,selectCategory,categoryName,categoryType,seo_title,url,seo_schema,image,onlyDate)VALUES('$userid','$username','$Title','$Content','$Status','$current_time','$Visibility',' $publihedOn','$Format','$selectCategory','$categoryName','$categoryType','$seo_title','$url','$seo_schema','$image','$current_time')";
...
if ($Status=='Draft') {
  $query="INSERT INTO posts(user_id,username,Title,Content,Status,draftTime,Visibility,publihedOn,Format,selectCategory,categoryName,categoryType,seo_title,url,seo_schema,image,onlyDate)VALUES('$userid','$username','$Title','$Content','$Status','$current_time','$Visibility',' $publihedOn','$Format','$selectCategory','$categoryName','$categoryType','$seo_title','$url','$seo_schema','$image','$current_time')";
    $insert_query=mysqli_query($connection,$query);
    if (!$insert_query) {
    printf("Error: %s\n", mysqli_error($connection));
    exit();
...
if(isset($_GET['page'])) {
          ...
          $post_query_count="SELECT * FROM trash";
...
// --------------------------
              $query="SELECT * FROM trash LIMIT $page_1,$per_page";
  ...
  <?php
   for($i =1; $i <= $count; $i++) {
        ...
        if($i == $page) {
             ...
             echo "<li class='page-item'><a class='page-link active_pagination' href='allpost.php?show=trash&&page={$i}'>{$i}</a></li>";


        }  else {

            echo "<li class='page-item''><a class='page-link' href='allpost.php?show=trash&&page={$i}'>{$i}</a></li>";
