...
$select_query1="SELECT * FROM category WHERE fid=$edit_id";
...
if ($statusdropdown == 'Draft') {
   $query="UPDATE posts SET Title='$addTitle',Content='$postContent',seo_title='$seo_title',description='$description',url='$url',seo_schema='$schema',draftTime='$current_time',Status='$statusdropdown', Visibility='$visible',publihedOn='$date',Format='$x',selectCategory='$selectCategory',categoryName='$category_name',categoryType='$category_type',image='$update_image' WHERE id={$edit_id}";
...
if ($statusdropdown == 'Publish') {
 ...
 $query="UPDATE posts SET Title='$addTitle',Content='$postContent',seo_title='$seo_title',description='$description',url='$url',seo_schema='$schema',publishTime='$current_time',Status='$statusdropdown', Visibility='$visible',publihedOn='$date',Format='$x',selectCategory='$selectCategory',categoryName='$category_name',categoryType='$category_type',image='$update_image' WHERE id={$edit_id}";
  $update_query=mysqli_query($connection,$query);
...
$fid_insert_query="UPDATE category SET selectCategory='$selectCategory',categoryName='$category_name',parentcategory='$category_type' WHERE fid={$edit_id}";
...
foreach($str_arr as $value){
  if ($value == $name) {
                                              ...
                                              ?>
      <input type="checkbox" name="select_category[]" value="<?php echo $name; ?>" <?php echo ($name == $value) ? "checked" : "" ; ?>/  class="mar_top_20"> <?php echo $name;    ?><br>
  ...
  while($row=mysqli_fetch_array($select_all_categories_query)){
                              $name=$row['categoryName'];
                              ?>
    <input type="checkbox" name="select_category[]" value="<?php echo $name; ?>"  class="mar_top_20"> <?php echo $name;    ?><br>                          
                              <?php
                            }
                              ?>
