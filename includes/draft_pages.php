...
if(isset($_GET['page'])) {
          ...
          $post_query_count="SELECT * FROM pages WHERE Status='Draft'";
...
<?php
              ...
              //$connection=mysqli_connect('localhost','root','','admin');
              $query="SELECT * FROM pages WHERE Status='Draft' LIMIT $page_1,$per_page";
  ...
  $createdDate=$row['createdDate'];
    ...
    <td> <a class="nav-link " href="#" id="actionmenu" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <div class="dropdown-menu dropdown-menu-right" id="actionmenulist" aria-labelledby="actionmenu">
          <a class="dropdown-item quickEditPost" href="edit_draft_pages.php?edit=<?php echo $id;   ?>">Edit<i class="fa fa-pencil pad_left_40"></i
            ></a>
  ...
  <?php
   for($i =1; $i <= $count; $i++) {
        ...
        if($i == $page) {
             ...
             echo "<li class='page-item active'><a class='page-link' href='allpages.php?show=draft&&page={$i}'>{$i}</a></li>";


        }  else {

            echo "<li class='page-item''><a class='page-link' href='allpages.php?show=draft&&page={$i}'>{$i}</a></li>";
