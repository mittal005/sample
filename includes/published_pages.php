...
if(isset($_GET['page'])) {
          ...
          $post_query_count="SELECT * FROM pages WHERE Status='Publish'";
...
<?php
              ...
              //$connection=mysqli_connect('localhost','root','','admin');
              $query="SELECT * FROM pages WHERE Status='Publish' LIMIT $page_1,$per_page";
  ...
  $createdDate=$row['createdDate'];
    ...
    <td> <a class="nav-link " href="#" id="actionmenu" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <div class="dropdown-menu dropdown-menu-right" id="actionmenulist" aria-labelledby="actionmenu">
          <a class="dropdown-item" href="edit_publish_pages.php?edit=<?php echo $id;   ?>">Edit<i class="fa fa-pencil pad_left_40"></i
            ></a>
          <a class="dropdown-item quickEditPost" data-title="Edit" data-id="<?php echo $id;  ?>">Quick Edit<i class="fa fa-magic pad"></i
            ></a>
  ...
  <?php
   for($i =1; $i <= $count; $i++) {
        ...
        if($i == $page) {
             ...
             echo "<li class='page-item'><a class='page-link active_pagination' href='allpages.php?show=published&&page={$i}'>{$i}</a></li>";


        }  else {

            echo "<li class='page-item''><a class='page-link' href='allpages.php?show=published&&page={$i}'>{$i}</a></li>";
